...
class StepsTests(unittest.TestCase):
    ...
    def test_step_say(self):
        ...
        with mock.patch.object(my_bot,
            ...
            step.say(bot=my_bot)
            mocked.assert_called_with('Current state: Level 1 - Initial capture of information')

            step = my_running_steps[1]
            step.say(bot=my_bot)
            mocked.assert_called_with('Current state: Level 2 - Escalation to technical experts')

            step = my_running_steps[2]
            step.say(bot=my_bot)
            mocked.assert_called_with('Current state: Level 3 - Escalation to decision stakeholders')

            step = my_running_steps[3]
            step.say(bot=my_bot)
            mocked.assert_called_with('Current state: Terminated - Process is closed, yet conversation can continue')
